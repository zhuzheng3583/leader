...
//探索者STM32F407开发板光盘资料\4.程序源码\3，扩展例程\1，普通扩展例程\2，库函数版本\扩展实验2 ATK-NEO-6M GPS模块实验
#include "gps.h"
#include "uart.h"
...
static S32 gps_open(struct device *this, S32 flags)
{
	...
	if(Ublox_Cfg_Rate(1000, 1) != 0)
	{
		...
		{
			dev->ioctl(uart_fops, CmdUartBanud, uartbanud);			//初始化串口3波特率为9600(EEPROM没有保存数据的时候,波特率为9600.)
...
#define GPS_SIZE 100
	...
	GPS_Analysis(buf,(u8*)gps_string);	//分析字符串
	//Gps_Msg_Show(); 					//显示信息
