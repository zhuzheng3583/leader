...
pwm::pwm(PCSTR name, s32 id) :
	device(name, id),
	_channel(0),
	_dutycycle(0)
{
	pwm_hw_table[_id].ppwm = this;
}

pwm::~pwm(void)
{
	pwm_hw_table[_id].ppwm = NULL;
}
...
s32 pwm::probe(void)
{
	...
	TIM_HandleTypeDef *htim = &pwm_hw_table[_id].TIM_Handle;
	...
	GPIO_TypeDef  *GPIOx = pwm_hw_table[_id].GPIOx; 
	GPIO_InitTypeDef *GPIO_Init= &pwm_hw_table[_id].GPIO_Init; 
	HAL_GPIO_Init(GPIOx, GPIO_Init);

	_irq = pwm_hw_table[_id].IRQn; 
	...
	TIM_OC_InitTypeDef *pTIM_OC_Init = &pwm_hw_table[_id].TIM_OC_Init;
...
s32 pwm::set_dutycycle(enum pwm_channel channel, u32 dutycycle)
{
	...
	TIM_TypeDef *TIMx = pwm_hw_table[_id].TIM_Handle.Instance;
