...
BOOL task::create(struct task_params *pparams)
{
 ...
 */
void task::msleep(s32 timeoutms)
{
    s32 error = OS_ERR_NONE;
	u32 tick = 0;
	if (timeoutms < 0) {
...
BOOL task::func(task* ptask)
{
	INF("START======================>\n");
	INF("name: %s.\n",          ptask->_params.name);
	INF("priority: %d.\n",      ptask->_params.priority);
	INF("stackbase: 0x%08x.\n", ptask->_params.stackbase);
	INF("stacksize: 0x%08x.\n", ptask->_params.stacksize);
	INF("END<========================\n");
